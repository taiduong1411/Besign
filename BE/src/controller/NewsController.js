...
const NewsController = {
    getNews: async (req, res, next) => {
        await News.find().lean().then(news => {
            return res.status(200).json(news)
        })
    },
    createNews: async (req, res, next) => {
        await News.findOne({ title: req.body.title }).then(async n => {
    ...
    useNews: async (req, res, next) => {
        await News.find({ status: true }).lean().limit(4).sort({ createdAt: -1 }).then(news => {
            return res.status(200).json(news)
        })
    },
    delNews: async (req, res, next) => {
        await News.findByIdAndDelete(req.params.id).then(async news => {
    ...
    getDetailBySlug: async (req, res, next) => {
        await News.findOne({ slug: req.params.slug }).then(news => {
            news = {
                title: news.title,
    ...
    getNewsByTag: async (req, res, next) => {
        try {
            ...
            let news = await News.find({ hashtags: regex }).skip((page - 1) * ITEMS_PER_PAGE)
    ...
    getAllData: async (req, res, next) => {
        try {
            ...
            const page = +req.query.page || 1;
            let news = await News.find().skip((page - 1) * ITEMS_PER_PAGE)
                .limit(ITEMS_PER_PAGE).sort({ createdAt: -1 });
            news = news.map(n => {
                return {
                    title: n.title,
